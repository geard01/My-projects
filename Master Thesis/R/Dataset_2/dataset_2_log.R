...
generated quantities {
  ...
  for (i in 1:N) {
    F_rep[i] = lognormal_rng(log(mu[i]), sigma);
    log_lik[i] = lognormal_lpdf(F[i] | log(mu[i]), sigma);  
  }
}
"
...
# Salva modello
model_file <- file.path("C:/Users/geard/OneDrive/Download/Tesi/STAN/Dataset_2", "bbi_model_log.stan")
...
results_file <- "C:/Users/geard/OneDrive/Download/Tesi/STAN/Dataset_2/stan_results_log.RData"
save(fit, stan_data, summary, diagnostics_list,
     file = results_file)

# Salva anche le diagnostiche in CSV
write.csv(summary, 
          file.path("C:/Users/geard/OneDrive/Download/Tesi/STAN/Dataset_2", "summary_log.csv"), 
          row.names = TRUE)

write.csv(ess_summary, 
          file.path("C:/Users/geard/OneDrive/Download/Tesi/STAN/Dataset_2", "diagnostics_log.csv"), 
          row.names = FALSE)
